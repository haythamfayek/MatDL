function [dX, dWLSTM, dc0, dh0] = lstm_backward(dHout_in, cache, dcn, dhn)
%LSTM_BACKWARD Compute the gradients of an LSTM layer
...
% Copyright (C) 2016-17 Haytham Fayek.
    ...
    % backprop the LSTM
    dIFOG = zeros(size(IFOG));
    dIFOGf = zeros(size(IFOGf));
    dWLSTM = zeros(size(WLSTM));
    dHin = zeros(size(Hin));
    dC = zeros(size(C));
    dX = zeros(M, D, T);
    dh0 = zeros(M, H);
