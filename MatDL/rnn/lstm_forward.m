function [out, cend, houtend, cache] = lstm_forward(x, h0, WLSTM, c0)
%LSTM_FORWARD Compute the forward pass for an LSTM layer
...
% Copyright (C) 2016-17 Haytham Fayek.
    ...
    Hin = zeros(M, xphpb, T); % input [1, xt, ht-1] to each tick of the LSTM
    out = zeros(M, H, T); % hidden representation of the LSTM (gated cell content)
    IFOG = zeros(M, H * 4, T); % input, forget, output, gate (IFOG)
    IFOGf = zeros(M, H * 4, T); % after nonlinearity
    C = zeros(M, H, T); % cell content
    Ct = zeros(M, H, T); % tanh of cell content
